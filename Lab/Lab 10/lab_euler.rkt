...
(define (get-vertices G)
  (cond
    [ (null? G) '()]
    [else (usort (list-all-vertices G))] 
  )
)
                          
(define (list-all-vertices G )
  (cond
    [ (null? G) '()]
    [else (append (append (list(caar G)) (cdar G) ) (list-all-vertices (cdr G)) ) ]
   )
  )
...
(define (degree G v)
  (cond
  [ (null? G) 0 ]
  [ (and (= v( caar G) ) (= v( cadar G) )) (+ 2 (degree (cdr G) v))]
  [ (= v( caar G) ) (+ 1 (degree (cdr G) v)) ]
  [ (= v( cadar G) ) (+ 1 (degree (cdr G) v)) ]
  [ else (degree (cdr G) v)]
 )
)
...
(define (all-even? G)
  (cond
    [ (null? G) #t]
    [else (all-even-helper G (get-vertices G) )]
    )
  )
...
(define (all-even-helper G V)
  (cond
    [ (null? V) #t]
    [else (if (zero? (modulo (degree G (car V)) 2) ) (all-even-helper G (cdr V)) #f ) ]
  )
)  
...
(define (adjacent G v)
  (cond
    [ (null? G) '()]
    [ (and (= v( caar G) ) (= v( cadar G) )) (usort (append (list v) (adjacent (cdr G) v)) )]
    [ (= v( caar G) ) (usort (append (list (cadar G)) (adjacent (cdr G) v)) ) ]
    [ (= v( cadar G) ) (usort (append (list (caar G)) (adjacent (cdr G) v))) ]
...
(define (connected? G)
   (cond
    [ (null? G) #t]
    [ else (connected-helper G (get-vertices G) (get-vertices G))]
  )
)  


(define(connected-helper G Vfull Vnext)
  (cond
    [ (null? Vnext) #t]
    [ else (if (equal? (usort (dfs G (car Vnext))) (get-vertices G)) (connected-helper G Vfull (cdr Vnext)) #f)]
  )
)  
