...
(define (indegree E v)
  (cond
    [( null? E) 0]
    [(equal? v (cadar E)) (+ 1 (indegree (cdr E) v))]
...
;; 5 PTS
(define (find-top G)
  (find-top-helper (get-vertices G) (get-edges G))
  )

(define (find-top-helper V-short E )
  (cond
    [(null? V-short) (topsort-error)]
    [(equal? (indegree E (car V-short)) 0) (car V-short) ]
    [ else (find-top-helper (cdr V-short) E)]
  )
)  
...
(define (pop-vertex G v)
  (make-graph (vertex-minus-v (get-vertices G) v) (edges-minus-v (get-edges G) v))
)


(define (vertex-minus-v V v)
  (cond
    [(null? V) '() ]
    [(equal? (car V) v) (vertex-minus-v (cdr V) v) ]
    [else (append (list (car V)) (vertex-minus-v (cdr V) v) ) ]
  )
 ) 

(define (edges-minus-v E v)
  (cond
    [ (null? E) '() ]
    [ (or (equal? (caar E) v) (equal? (cadar E) v)) (edges-minus-v (cdr E) v) ]
    [else (append (list(car E)) (edges-minus-v (cdr E) v) ) ]
  )  
 )  
