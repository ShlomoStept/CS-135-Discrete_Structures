...
(define (reflexive? n R)
  (cond
    [ (disjoint?   [list(append ( cons n '() ) ( cons n '() ) ) ]   R  ) #f ]
    [ else #t ]
  )
)  
...
(define (relate x R)
  (cond
    [ (null? R) '() ]
    [ (equal? (caar R) x) (append (cdar R) (relate x (cdr R) )  ) ] 
    [ else (relate x (cdr R) ) ]
   ) 
)
...
(define (get-y_s-of-subsets S)
  (cond
    [ (null? S)  '()]
    [ else ( append (cdar S) (get-y_s-of-subsets (cdr S)) ) ]
  )
)
...
(define (injective? R)
  (if (double? (get-y_s-of-subsets R)) #f #t) 
    )
