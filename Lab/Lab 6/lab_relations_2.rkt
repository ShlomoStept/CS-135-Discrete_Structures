...
(define (compose S R)
  (cond
    [ (null? S) '() ]
    [ (null? R) '() ]
    [ (containsY? (cadar R) S) (union (XgrabY (cadar R) (caar R) S) (compose S (cdr R)) ) ]
    [ else (compose S (cdr R)) ]                               

    )
)

(define (containsY? x S)
    (cond
      [(null? S) #f]
      [(= x (caar S)) #t]
      [else (containsY? x (cdr S))]
      )
  )
...
(define (XgrabY a x R)
    (cond
      [(null? R)'()]
      [(= a (caar R)) ( cons (cons x (cdar R)) (XgrabY a x (cdr R))  ) ]
      [else (XgrabY a x (cdr R))]
      )
  )
...
(define (transitive-closure R)
  (sucessive-runs ( SubsetCardinality R) R)
)


(define (SubsetCardinality R)
  (cond
    [ (null? R) 0]
    [ else (+ 1 (SubsetCardinality (cdr R) ) ) ]
  )
)

(define(sucessive-runs x R)
  (cond
    [ (= x 0) '() ]
    [ else (union (sucessive-runs (- x 1) R) (power x R) ) ]
    )
  )
...
(define (transitive? R)
  (cond
    [  (set-equal? (transitive-closure R) R ) #t ]
    [else #f ]
  )
)
...
(define (equivalence? n R)
 (cond
   [ (set-equal? (union (union (reflexive-closure n R) (symmetric-closure (reflexive-closure n R)) ) (transitive-closure (symmetric-closure (reflexive-closure n R))))  R ) #t]
   [else #f ]
 )
)
...
(define (involutive? n R)
  (if (and (test1 n R)
          (and (injective? R) (injective? (converse R)))
          (and (containsX? n R) (equal? (intersection   R (XgrabY_Symetry n R) ) (XgrabY_Symetry n R)))) #t #f))


  (define (containsX? x S)
    (cond
      [(null? S) #f]
      [(= x (caar S)) #t]
      [else (containsX? x (cdr S))]
      )
  )
(define (test1 n R)
         (if (equal? ( intersection R (cons (cons n (list n) ) '() )) (cons (cons n (list n)) '() ))
             (null?  (difference  (id n) (intersection  R (power n R ))  )) (equal? R R) ) ) 

(define (XgrabY_Symetry n R)
    (cond
      [(null? R)'()]
      [(= n (caar R)) (make-set(append(cons (cons n (cdar R)) (cons (append (cdar R) (list n) ) '() ))))]
      [else (XgrabY_Symetry n (cdr R))]
      )
  )
