...
(define (p-prime? p)
  (cond
   ...
   [ (p= p (succ z)) #f]
   [ (p= p (succ(succ z)) ) #t]
   [ (p-prime-helper (pred p) p)  #t ]
   [else #f]
   )
 ) 

(define(p-prime-helper p s)
   (cond
   [ (and (p= p (succ(succ z))) (not(p-divides? (succ(succ z)) s))  ) #t]
   [ (and (not (p-divides? p s )) (p-prime-helper (pred p) s)) #t ]
   [else #f]
   )
)  