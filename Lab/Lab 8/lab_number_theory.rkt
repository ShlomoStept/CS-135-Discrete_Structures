...
(define (divides? a b)
  (cond
    [ (zero? a) (if (zero? b) #t #f)]
    [ else (if (zero? (modulo b a)) #t #f)]
    )
  )
...
(define (gcd a b)
  (cond
    [ (or (zero? a) (zero? b)) (if (zero? a) b a) ]
    [else (if ( < a b) (gcd-helper a b) (gcd-helper b a) )] 
  )
)
(define (gcd-helper x y)
  (cond
    [ (zero? (modulo y x)) x]
    [else (gcd-helper (modulo y x) x) ]
  )
)
...
(define (wj-solvable? cA cB g)
  (cond
    [ (and (divides? (gcd cA cB) g) (or (<= g cA) (<= g cB))) #t]
    [ else #f ]
  )
)
...
(define (waterjug cA cB g)
    [ append (append '((0 0)) (waterjug-helper cA cB g 0 0)) ]
  )

( define (waterjug-helper cA cB g fA fB)
   (cond 
    [ (equal? fB g) '() ]
    [ (equal? fB cB)  (append (cons (list fA 0) '()) (waterjug-helper cA cB g fA 0))]
    [ (zero? fA) (append (cons (list cA fB) '()) (waterjug-helper cA cB g cA fB)) ]
    [ (<  cB (+ fA fB)) (append (cons (list (- cA (- cB fB)) cB) '()) (waterjug-helper cA cB g (- cA (- cB fB)) cB)  ) ]
    [ (<= (+ fA fB) cB) (append (cons  ( list 0 (+ fA fB) ) '() ) ( waterjug-helper cA cB g 0 (+ fA fB) )  ) ]
   )
 )  
...
(define (smallest-p n p)
  (cond
    [ (= n 1) '()]
    [  (divides? p n) (append (list p) (smallest-p (/ n p) 2))]
    [ else (smallest-p n (+ p 1)) ]
  )
)
