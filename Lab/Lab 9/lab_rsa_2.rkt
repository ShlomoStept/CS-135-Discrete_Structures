...
(define (generate p q e)
  (append (cons (list (* p q) e) '()) (cons (list (* p q) (mul-inv e (lcm (- p 1) ( - q 1)) ) ) '()  ) )
)
...
(define (mod-exp b e m)
  (cond
   [ (= e 0) 1]
   [ ( and (> e 0) (even? e)) (modulo ( power (mod-exp b (/ e 2) m) 2 ) m) ]
   [ ( and (> e 0) (odd? e)) (modulo ( * (mod-exp b (- e 1) m) b ) m) ]
  )
)
...
(define (power a b)
  (cond
    ...
    [ (= b 2) ( * a a)]
    [ (= b 1) a]
    [ (even?  b) (* (power a (/ b 2)) (power a (/ b 2)) )    ]
    [ else ( * a (power a (- b 1) )) ]
  )
)
...
(define (encrypt-helper plaintext key)
  (cond
    [(null? plaintext) '()]
    [ else (append (list (mod-exp (char->integer (car plaintext)) (cadr key) (car key)) ) (encrypt-helper (cdr plaintext) key) ) ]
  )
)  
...
(define (decrypt-helper ciphertext key)
  (cond
     [(null? ciphertext) '()]
    [ else (append (list(integer->char (mod-exp (car ciphertext) (cadr key) (car key)) )) (decrypt-helper (cdr ciphertext) key) ) ]
  )
)  
